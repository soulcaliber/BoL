...
function SpellDmgLibGetVersion ()
	return 1.32
end
...
spellDmg = {
	...
	Diana = {
		PDmgM = "math.max(5*lvl+15,10*lvl-10,15*lvl-60,20*lvl-125,25*lvl-200)+.6*ap", -- (bonus)
		PType = 2,
	...
	Elise = {
		...
		QDmgM = "40*Qlvl+(8+.03*ap)*thp/100",
		QMDmgM = "50*Qlvl+10+(8+.03*ap)*(tmhp-thp)/100",
	...
	MissFortune = {
		...
		WDmgM = "math.max(2*Wlvl+2+.05*ap,(2*Wlvl+2+.05*ap)*5*stagedmg3)", --xstack (5). stage3: Max damage
	...
	Rengar = {
		PType = 2,
		QDmgP = "math.max((30*Qlvl)*stagedmg1,(30*Qlvl+ad)*stagedmg2,(30*Qlvl+ad)*stagedmg3)", --stage1:Savagery. stage2:Empowered Savagery . stage3: Empowered Savagery
		QType = 2,
	...
	Shyvana = {
		...
		EDmgM = "math.max((35*Elvl+45+.6*ap)*(stagedmg1+stagedmg3),(2*tmhp/100)*stagedmg2)", --stage1:Active. stage2:Each autoattack that hits debuffed targets. stage3:Active
	...
	Thresh = {
		...
		EDmgM = "math.max((40*Elvl+25+.4*ap)*(stagedmg1+stagedmg3),((.3*Qlvl+.5)*ad)*stagedmg2)", --stage1:Active. stage2:Passive (Souls + up to ...). stage3:stage1
		--
...
function getDmg(spellname,target,owner,stagedmg,spelllvl)
	...
	elseif (spellname == "TRINITY") then
		TrueDmg = owner:CalcDamage(target,(ad-bad)*2) --(bonus)
	elseif (spellname == "LICHBANE") then
		TrueDmg = owner:CalcMagicDamage(target,50+.75*ap) --(bonus)
	...
	elseif (spellname == "ICEBORN") then
		TrueDmg = owner:CalcDamage(target,(ad-bad)*1.25) --(bonus)
	...
	elseif (spellname == "SUNFIRE") then
		TrueDmg = owner:CalcMagicDamage(target,40) --x sec
